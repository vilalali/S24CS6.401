...
public class UserBookDao {
    ...
    public UserBook getByBookid(String bookId) {
        System.out.println(bookId + "bookId bookId book ID value is here");
        EntityManager em = ThreadLocalContext.get().getEntityManager();
        Query q = em.createQuery("select ub from UserBook ub where ub.bookId = :bookId and ub.deleteDate is null");
        q.setParameter("bookId", bookId); 
        try {
            return (UserBook) q.getSingleResult();
        } catch (NoResultException e) {
    ...
    public void findByCriteria(PaginatedList<UserBookDto> paginatedList, UserBookCriteria criteria, SortCriteria sortCriteria) throws Exception {
        ...
        StringBuilder sb = new StringBuilder("select ub.UBK_ID_C c0, b.BOK_TITLE_C c1, b.BOK_SUBTITLE_C c2, b.BOK_AUTHORS_C c3, b.BOK_LANGUAGE_C c4, b.BOK_PUBLISHDATE_D c5, ub.UBK_CREATEDATE_D c6, ub.UBK_READDATE_D c7");
        ...
        if (!Strings.isNullOrEmpty(criteria.getSearch())) {
            criteriaList.add(" (b.BOK_TITLE_C like :search or b.BOK_SUBTITLE_C like :search or b.BOK_AUTHORS_C like :search) ");
        ...
        for (Object[] o : l) {
            ...
            userBookDto.setAuthors((String) o[i++]);
