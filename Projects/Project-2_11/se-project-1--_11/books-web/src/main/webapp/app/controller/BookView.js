...
App.controller('BookView', function($scope, $q, $timeout, $state, $stateParams, Restangular) {
  ...
  $scope.rateBook = function(rating) {
    if (rating && $scope.book && $scope.book.id) {
      Restangular.one('book', $scope.book.id).post('rate', {rating: rating})
        .then(function(response) {
          // Update the frontend with the new rating, if needed
          // For example, if your backend returns the updated book object
          $scope.book.rating = response.rating;
          alert('Thank you for rating this book!');
        }, function(error) {
          console.error('Failed to rate the book', error);
